...
public class OptionRequest extends BodyRequest<OptionRequest> {
    ...
    protected void prepare() {
        if (mRequestBody != null) {
            mObservable = getClient().getClient().create(RetrofitAPI.class).optionsBody(mUrl, mRequestBody);
            ...
            mObservable = getClient().getClient().create(RetrofitAPI.class).options(mUrl);
        }
    }
